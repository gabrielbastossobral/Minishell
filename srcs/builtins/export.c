...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/03/06 18:49:53 by gabrielsobr       #+#    #+#             */
/*   Updated: 2025/03/06 19:52:21 by gabrielsobr      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../includes/minishell.h"

static	int	get_var_pos_equal(char *var, char **envp)
{
	char	*var_temp;
	int		var_len;
	int		var_pos;
...
static void	update_var_array(char *var, int var_pos, char ***envp)
{
	char	**envp_temp;
	...
	if (!envp[0][var_pos])
	{
		...
		while (var_pos--)
			envp_temp[var_pos] = ft_strdup ((*envp)[var_pos]);
		*envp = free_mat (*envp);
		*envp = envp_temp;
	}
	else
...
void	ft_export(t_data *ms, char **cmd, char ***envp)
{
	...
	while (cmd[i])
	{
		...
		{
			var_pos = get_var_pos_equal (cmd[i], *envp);
			update_var_array (cmd[i], var_pos, envp);
			ms -> error = 0;
		}
